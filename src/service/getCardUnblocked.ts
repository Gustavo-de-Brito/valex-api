...
async function getCardUnblocked(cardId: number, password: string) {
  // card validations
  const cardData:any = await isCardValid(cardId);
  isExpirationDateValid(cardData.expirationDate);
  isCardUnblocked(cardData.isBlocked);
  isPasswordValid(cardData.password, password);

  await cardRepository.unblockCard(cardId);
}

export default getCardUnblocked;